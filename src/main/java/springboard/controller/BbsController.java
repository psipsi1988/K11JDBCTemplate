...
/*
@Autowired
	: 스프링 설정 파일에서 생성된 빈을 자동으로 주입받을 때 사용하는 어노테이션
	-생성자, 필드(멤버변수), 메소드(setter)에 적용 가능
	-setXXX()의 형식이 아니어도 적용 가능
	-타입을 이용해 자동으로 프로퍼티의 값을 설정. 
	-따라서 빈을 주입받을 객체가 존재하지 않거나 
		같은 타입이 2개 이상 존재하면 예외가 발생됨.
	-해당 어노테이션은 멤버변수에만 적용할 수 있다. 
	함수 내의 지역 변수에서는 사용할 수 없다. 
...
public class BbsController {
	 ...
	 * template; System.out.println("@Autowired->JDBCTemplate 연결 성공");
	 * 
	...
	멤버변수이므로 클래스 내에서 전역적으로 사용한다. 
	해당 클래스의 모든 command객체는 위 인터페이스를 구현하여 정의하게 된다. 
	 */
	...
	아래와 같이 자동 주입 받아서 사용하는 것이 스프링 개발방법론에 
	부합하는 방법이다. 
	 */
	...
	public String list(Model model, HttpServletRequest req) {
		...
		이른(?) 커맨드객체로 전달하기 위해 model에 저장 후 매개변수로 전달한다. 
		 */
		model.addAttribute("req", req);
		
		/*
		컨트롤러는 사용자의 요청을 분석한 후 해당 요청에 맞는 서비스객체만 
		호출하고 실제 DAO의 호출이나 비즈니스로직은 아래 Command객체가 처리한다. 
		 */
	...
	/*
	글쓰기처리 : post로 전송되므로 두 가지 속성을 모두 사용하여 매핑
	 */
	...
	public String writeAction(Model model,
		...
		//request객체를 모델에 저장
		model.addAttribute("req", req);
		//View에서 전송할 폼값을 한꺼번에 저장한 커맨드객체로 DTO를 저장
	...
	public String passwordAction(Model model, HttpServletRequest req) {
		...
		if(rowExist<=0) {
			...
			if(mode.equals("edit")) {
				...
				/*
				컨트롤러에서 뷰의 경로를 반환할 때 아래와 같이 redirect하게 되면 
				list.do?noPage=10 이와 같이 URL을 자동으로 조립해서 
				로케이션시켜준다. 
				 */
	...
	public String editAction(HttpServletRequest req, 
		...
		이동시 쿼리스트링 형태로 redirect하지 않고 model객체에 필요한 파라미터를 
		저장하면 자동으로 URL을 조립하여 View를 호출해준다. 
		 */
		model.addAttribute("idx", req.getParameter("idx"));
		model.addAttribute("nowPage", req.getParameter("nowPage"));
