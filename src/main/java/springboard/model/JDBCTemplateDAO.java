...
import springboard.controller.BbsController;
...
- List query(String sql, Object[] args, RowMapper rowMapper)
	: 인파라미터를 가진 여러 개의 레코드를 반환하는 
	select 계열의 쿼리문인 경우 사용한다. 
	
-int queryForInt(String sql) 혹은 queryForInt(String sql, Object[] args)
	: 쿼리문의 실행결과가 숫자를 반환하는 select계열의 쿼리문에 사용한다. 
-Object queryForObject(String sql, RomMapper rowMapper)
	혹은 Object querForObject(String sql, Object[] args, RowMapper rowMapper)
	: 하나의 레코드를 반환하는 select계열의 쿼리문 실행시 사용한다. 
...
public class JDBCTemplateDAO {
	...
	//setter메소드 생성 (Spring-JDBC 2차 버전)
	/*
	...
	public JDBCTemplateDAO() {
		/*
			...
			즉, DB연결정보를 애플리케이션 어디서든 사용할 수 있다. 
		 */
	...
	public void write(final SpringBbsDTO springBbsDTO) {
		template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) 
				...
				또한 nextVal은 한 문장에서 여러 번 사용하더라도 같은 시퀀스를 반환한다. 
				 */
	...
	public void updateHit(final String idx) {
		...
		반드시 final로 선언해야 사용 가능하다. (자바의 규칙)
		 */
	...
	public SpringBbsDTO view(String idx) {
		...
		queryForObject()메소드는 반환결과가 0개이거나 2개 이상인 경우
		 	예외가 발생하므로 반드시 예외 처리를 해주는 적이 좋다. 
		 */
		try {
	...
	public int password(String idx, String pass) {
		...
		예외 처리를 하고 있다. 
		queryForObject()는 반환되는 행이 1개일 때만 정상 동작한다. 
		 */
		try {
			SpringBbsDTO dto = template.queryForObject(sql, 
					new BeanPropertyRowMapper<SpringBbsDTO>(
							SpringBbsDTO.class));
			/*
			idx와 pass에 해당하는 게시물이 정상적으로 가져왔을 때는 
			해당 idx값을 반환값으로 사용한다. 
			 */
			retNum = dto.getIdx();
		}
			...
			/*
			만약  일치하지 않아 예외가 발생되면 0을 반환한다. 
			일련번호는 시퀀스를 사용하므로 항상 0보다 큰값을 가지게 된다. 
			 */
	...
	public void edit(final SpringBbsDTO dto) {
		/*
		해당 게시판에서 패스워드는 변경의 이 아니라 검증의 대상으로만 사용됨.
		...
		/*
		매개변수 dto객체를 아래 익명 클래스 내부에서 사용해야 하므로 
		반드시 final을 붙여줘야 한다. 
		 */
		template.update(sql, new PreparedStatementSetter() {
