...
/*
BbsCommandImpl을 구현하였으므로 execute()메소드는 반드시 오버라이딩해야 한다.
또한 해당 객체는 부모타입인 BbsCommandImpl로 참조할 수 있다.  
 */

/*
Service역할의 클래스임을 명시함
Service객체는 Controller와 Model 사이에서 중재역할을 함
 */
@Service
public class ListCommand implements BbsCommandImpl{
	...
	public void execute(Model model) {
		...
		//출력할 리스트 가져오기
		//ArrayList<SpringBbsDTO> listRows = dao.list(paramMap);//페이지 X
		ArrayList<SpringBbsDTO> listRows = dao.listPage(paramMap);//페이지 O
		...
		for(SpringBbsDTO row : listRows) {
			//전체 게시물의 개수에서 하나씩 차감하면서 가상번호 부여
