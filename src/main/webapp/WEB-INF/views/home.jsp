...
3. 요청명을 결정하고 컨트롤러를 생성한다. 
	setvlet-context.xml에서 기본패키지를 아래와 같이 추가함. (반드시 필요한 건 아님. )
	<context:component-scan base-package="springboard" />
	
4. Service객체, DAO객체를 생성한다. 
	Service는 컨트롤러와 모델 사이에서 중재 역할을 하는 객체로써 
	컨트롤러의 모든 요청을 서비스로 넘겨주기 위해 '커맨드객체'를 사용한다. 
	request객체를 model객체에 저장한 후 서비스객체로만 model을 매개변수로 전달한다. 
	서비스객체에서는 model.asMap() 메소드를 통해 Map컬렉션으로 변환한 후
	요청들을 처리하고 모델을 호출한다. 

5. 게시판 구현에 필요한 객체들을 new를 통해 생성하지 않고 
	주입(DI)을 통해서 구현한다.  이때 @Autowired를 사용한다. 
